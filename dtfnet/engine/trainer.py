...
from dtfnet.utils.comm import get_world_size, synchronize
from dtfnet.utils.metric_logger import MetricLogger
...
#                 param.requires_grad_(True)
        ...
        for iteration, (batches, idx) in enumerate(data_loader):
            ...
            optimizer.zero_grad()
            contr_weight = cfg.MODEL.DTF.LOSS.CONTRASTIVE_WEIGHT
...
#                 loss += (loss_sent + loss_vid) * 0.01
        ...
        if checkpoint_period != -1 and epoch % checkpoint_period == 0:
            checkpointer.save(f"{cfg.MODEL.DTF.FEAT2D.NAME}_model_{epoch}e", **arguments)
