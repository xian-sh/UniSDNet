import logging
import torch
from dtfnet.data.datasets.evaluation import evaluate
...
def inference(
    ...
    logger.info(
        "Model inference time: {} ({:.03f} s / inference per device, on {} devices)".format(
            total_infer_time,
            inference_timer.total_time * num_devices / 17031.0,
            num_devices,
        )
    )
    ...
    if not is_main_process():
        return
    '''
    return evaluate(cfg, dataset=dataset, predictions=predictions, nms_thresh=nms_thresh, epoch=epoch)
