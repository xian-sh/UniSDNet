...
#             self.ious.append(all_iou2d)
    ...
    def __getitem__(self, idx):
        vid = self.data[idx]
        duration, timestamps, audios_name, sentences = self.annos[vid]['num_frames']/self.annos[vid]['fps'], self.annos[vid]['timestamps'], \
        ...
        all_iou2d = []
        for timestamp in timestamps:
            time = torch.Tensor([max(timestamp[0]/self.annos[vid]['fps'], 0), min(timestamp[1]/self.annos[vid]['fps'], duration)])
            iou2d = moment_to_iou2d(time, self.num_clips, duration)
            moments.append(time)
            all_iou2d.append(iou2d)
        moments = torch.stack(moments)
...
#         sent = [sentences[i] for i in index]
    ...
    def get_sentence(self, idx):
        vid = self.data[idx]
        duration, timestamps, audios_name = self.annos[vid]['num_frames']/self.annos[vid]['fps'], self.annos[vid]['timestamps'], \
            self.annos[vid]['audios']
        
        sentence = self.annos[vid]['sentences']
        moment = [torch.tensor([max(x[0]/self.annos[vid]['fps'], 0), min(x[1]/self.annos[vid]['fps'], duration)]) for x in timestamps]
...
#         sentence = self.annos[vid]['sentences']
    ...
    def get_num_audios(self, idx):
        vid = self.data[idx]
        duration, timestamps, audios_name = self.annos[vid]['num_frames'], self.annos[vid]['timestamps'], \
            self.annos[vid]['audios']
