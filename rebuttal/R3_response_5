...
Thank you very much for your recognition of our contributions to NLVG and SLVG tasks. Your constructive comments are exceedingly helpful for us to improve our paper. In the following, your comments are first stated and then followed by our point-by-point responses.
...
where $x^n$ represents the perception condition in the current state, $x^{n+1}$ denotes the reflection result, and $\text{ResMLP}(x^n)$ can enhance the information of the perception condition $x^n$.
...
[R16] Cleeremans et al., Consciousness matters: Phenomenal experience has functional value. Neuroscience of consciousness. 2022, niac007 (2022).

[R17] Richards et al., A deep learning framework for neuroscience. Nature neuroscience, 22, 1761-1770, 2019.

[R18] Grover et al., Differential mechanisms underlie trace and delay conditioning in Drosophila. Nature, 603, 302-308, 2022. 
...
To meet the constraint of nonlinear correlated Gaussian kernels, we randomly set different biases $z_k$ at equal intervals (e.g., 0.1 or 0.2) starting from 0.0, sweep the value of $h$ from 25 to 200 (see the ablation experiment Table 9 in appendix and the best setting is $h=50$) and set the global range of $\{z_k\}$ values to $[0, 5]$ in our experiments.

As per your valuable suggestion, we add new experiments about different $\gamma$ in **Table R4**. Gaussian coefficient $\gamma$ reflects the amplitude of the Gaussian kernel function that controls the gradient descent speed of the function value. 
It can be found that from **Table R4**, when $\gamma=25.0$, our model achieves the best performance. We also list the average and standard deviation of the five experimental results of $\gamma=\{5.0, 10.0, 25.0, 50.0, 70.0\}$. 
We select $\gamma=10.0$ as the empirical setting as its result is closest to the average \underline{avg.}. The optimal settings of variables ($h,\gamma$) are set to 50 and 25.0, and $z_k$ is set at equal interval 0.1. 
We will add this new ablation to the paper.
...
| 10.0 | 75.85  | 60.75 | 38.88 | 91.16 | 85.34 | 74.02 | 55.47 |
...
As shown in **Table R5**, our DTFNet consistently achieves the best performance compared to other graph settings, such as achieving 39.30 at the metric R@1, IoU@0.7. 
